...
Session::Session (Player *player1, Player *player2,
				  QStringList reckoChoosenWords, QObject *parent)
...
void
Session::sendMessageToPlayer1 (QString message)
{
...
void
Session::sendMessageToPlayer2 (QString message)
{
...
void
Session::sendMessageToBothPlayers (QString message)
{
...
Session::startRecko ()
{
  connect (player1->tcpSocket, &QTcpSocket::readyRead, this,
		   &Session::player1ReadyReadRecko);
  connect (player2->tcpSocket, &QTcpSocket::readyRead, this,
		   &Session::player2ReadyReadRecko);
}
...
Session::stopRecko ()
{
  ...
  disconnect (player1->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player1ReadyReadRecko);
  disconnect (player2->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player2ReadyReadRecko);
...
Session::stopMojBroj ()
{
  ...
  disconnect (player1->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player1ReadyReadMojBroj);
  disconnect (player2->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player2ReadyReadMojBroj);
...
Session::stopKoZna ()
{
  ...
  disconnect (player1->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player1ReadyReadKoZna);
  disconnect (player2->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player2ReadyReadKoZna);
...
//
void
Session::stopPogodiSta ()
{
  qDebug () << "Zavrsila se igra Pogodi Sta, prelazimo na sledecu igru :)"
			<< endl;
  disconnect (player1->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player1ReadyReadPogodiSta);
  disconnect (player2->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player2ReadyReadPogodiSta);

  startMemorija ();
}

void
Session::startMojBroj ()
{
...
void
Session::stopPodrunda ()
{
  disconnect (player1->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player1ReadyReadPodrunda);
  disconnect (player2->tcpSocket, &QTcpSocket::readyRead, this,
			  &Session::player2ReadyReadPodrunda);
...
void
Session::startMemorija ()
{
  connect (player1->tcpSocket, &QTcpSocket::readyRead, this,
		   &Session::player1ReadyReadMemorija);
  connect (player2->tcpSocket, &QTcpSocket::readyRead, this,
		   &Session::player2ReadyReadMemorija);
}

void
Session::stopMemorija ()
{
  ...
  saveResult (player1->getPlayerUsername (), player1->getPoints (),
			  player2->getPlayerUsername (), player2->getPoints ());
...
void
Session::processReckoMessage (const QString &msg)
{
	  ...
	  if (result == "GGGGG")
		{
		  sendMessageToBothPlayers (
			  ...
			  + QString::number (reckoGameNo) + "_ENDED\n");
		  if (reckoGameNo == 1)
			{
...
void
Session::processPodrundaMessage (const QString &msg, const int num)
{
  ...
  if (player1->podrunda_time != -1 & player2->podrunda_time != -1
	  & player1->podrunda_guess != -1 & player2->podrunda_guess != -1
	  & player1->podrunda_resenje != -1 & player2->podrunda_resenje != -1)
	{
	  ...
	  if (player1->pobednik & player2->pobednik)
		{
		  sendMessageToBothPlayers (
			  "POENI1:" + QString::number (player1->pointsKoZna) + "\n");
		  sendMessageToBothPlayers (
			  "POENI2:" + QString::number (player2->pointsKoZna) + "\n");
		}
...
void
Session::processMemorijaMessage (const QString &request)
{
...
void
Session::shuffleQVector (QVector<int> &vector)
{
...
QString
Session::checkReckoSolution (const QString &word)
{
  QString result;
  qDebug () << word << endl;
  qDebug () << recko << endl;
...
void
Session::processMojBrojMessage (const QString &msg)
{
  ...
  else if (msg.startsWith ("EXPRESSION:"))
	{
	  ...
	  if (expression.startsWith (player1->getPlayerUsername ()))
		sendMessageToPlayer2 (
			"EXPRESSION:"
			+ expression.mid (player1->getPlayerUsername ().length () + 1)
			+ "\n");
	  else
		sendMessageToPlayer1 (
			"EXPRESSION:"
			+ expression.mid (player2->getPlayerUsername ().length () + 1)
			+ "\n");
	}
...
void
Session::processKoZnaMessage (const QString &msg, int num)
{
...
void
Session::checkMojBrojSolution (const QString &pt1, const QString &pt2)
{
  qDebug () << "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ "
			<< submit_mojbroj;
  ...
  else if ((res2 == -1179 || res2 == -1951)
		   && (res1 != -1179 && res1 != -1951))
	{
	  sendMessageToBothPlayers ("POINTS:10%0");
	  return;
	}
  else if ((res1 == -1179 && res2 == -1951) || (res1 == -1951 && res2 == -1179)
		   || (res1 == -1179 && res2 == -1179)
		   || (res1 == -1951 && res2 == -1951))
	{
	  qDebug () << "PRAZNO!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@";
...
Session::generateInitialNumbers ()
{
  ...
  initialNumbers = initialNumbers + "-"
				   + QString::number (fifthNumberOptions[fifthNumber]);

  int sixthNumberOptions[] = { 25, 50, 75, 100 };
  int sixthNumber = QRandomGenerator::global ()->bounded (3);
  initialNumbers = initialNumbers + "-"
				   + QString::number (sixthNumberOptions[sixthNumber]);
  ...
  if (!QFile::exists (filePath))
	{
	  ...
	  QString pitanjce = "a_b_c_d_e_f/na_b_c_d_e_f/na_b_c_d_e_f/na_b_c_d_e_f/"
						 "na_b_c_d_e_f/na_b_c_d_e_f/na_b_c_d_e_f";
...
QString
Session::checkKoZnaSolution (const QString &word)
{
  QString result;
  qDebug () << word << endl;
  qDebug () << recko << endl;
...
void
Session::processPogodiStaMessage (const QString &msg, Player *player)
{
  if (msg.startsWith ("IMAGE_GEN"))
  ...
  else if (msg.startsWith ("IMG_GUESS:"))
	{
	  ...
	  if (guess == pogodiSta.toLower ())
		{
		  QString answer = "IMG_ANS:" + QString::number (player->playerId)
						   + ":" + pogodiSta + "\n";
...
void
Session::saveResult (const QString &player1Name, int player1Points,
  ...
  if (file.open (QIODevice::Append | QIODevice::Text))
	{
	  ...
	  stream << player1Name << "," << player1Points << "," << player2Name
			 << "," << player2Points << ","
