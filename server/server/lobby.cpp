...
Lobby::addPlayer (Player *player)
{
  ...
  qDebug () << "Dodat je novi igrac u Lobby. Sada ih ima: "
			<< getPlayersCount ();
  ...
  if (getPlayersCount () >= 2)
	{
	  ...
	  players.removeFirst ();
	  players.removeFirst ();
	  qDebug () << "Uklonili smo igrace iz Lobby-ja nakon zavrsetka igre. "
				   "Sada ih ima: "
				<< getPlayersCount ();
	}
}
...
const QList<Player *> &
Lobby::getPlayers () const
{
  return players;
}

int
Lobby::getPlayersCount () const
{
  return players.size ();
}

QStringList
Lobby::loadWordsFromFile (const QString &filePath)
{
  QStringList words;
...
QStringList
Lobby::chooseRandomWords (const QStringList &wordList)
{
