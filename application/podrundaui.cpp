...
Podrundaui::Podrundaui (QWidget *parent)
	: QWidget (parent), ui (new Ui::Podrundaui)
{
  getUi ()->setupUi (this);
  setBackground ();
  ...
  connect (getUi ()->pbOdgovori, &QPushButton::clicked, this,
		   &Podrundaui::on_pbOdgovori);
...
Podrundaui::Podrundaui (QWidget *parent, QTcpSocket *tcpSocket,
						QString playerr1, QString playerr2, int playerr1Points,
						int playerr2Points)
	: QWidget (parent), ui (new Ui::Podrundaui), podrunda (new Podrunda ()),
	  timer (new QTimer (this))
{
...
void
Podrundaui::startGame ()
{
  connect (server, SIGNAL (readyRead ()), this, SLOT (onReadyRead ()));
  timer->start (1000);

  connect (getUi ()->pbOdgovori, &QPushButton::clicked, this,
		   &Podrundaui::on_pbOdgovoriMultiplayer);
...
QString
Podrundaui::getPlayer1 ()
{
  return player1;
}

QString
Podrundaui::getPlayer2 ()
{
  return player2;
}

void
Podrundaui::setPlayer1 (QString playerr1)
{
  player1 = playerr1;
}

void
Podrundaui::setPlayer2 (QString playerr2)
{
  player2 = playerr2;
}

int
Podrundaui::getPlayer1Points ()
{
  return player1Points;
}

int
Podrundaui::getPlayer2Points ()
{
  return player2Points;
}

void
Podrundaui::setPlayer1Points (int playerr1Points)
{
  player1Points = playerr1Points;
}

void
Podrundaui::setPlayer2Points (int playerr2Points)
{
  player2Points = playerr2Points;
}
...
void
Podrundaui::getQuestion ()
{
  notifyServer (server, "PITANJE!\n");
}

void
Podrundaui::displayQuestion ()
{
  enableUI ();
  const QString pitanje = podrunda->getAnswerQuestion ().first;
  const QString resenje
	  = QString::number (podrunda->getAnswerQuestion ().second);
...
void
Podrundaui::notifyServer (QTcpSocket *socket, QString msg)
{
...
Podrundaui::on_gameEnded ()
{
  ...
  const QString poruka
	  = "Niste odgovorili na pitanje! Tacan odgovor je: " + correctAnswer;
...
void
Podrundaui::adjustResultLabel ()
{
  QFontMetrics metrics (getUi ()->labTacanOdgovor->font ());
  QSize textSize
	  = metrics.size (Qt::TextSingleLine, getUi ()->labTacanOdgovor->text ());
  getUi ()->labTacanOdgovor->setFixedSize (textSize);
}

void
Podrundaui::onReadyRead ()
{
...
void
Podrundaui::processServerMessage (QString serverMessage)
{
  if (serverMessage.startsWith ("PITANJE:"))
	{
	  QString pitanje = serverMessage.mid (8);
	  QStringList pitanje_odgovor = pitanje.split (',');
	  podrunda->setAnswerQuestion (pitanje_odgovor.value (0),
								   pitanje_odgovor.value (1).toDouble ());
  ...
  else if (serverMessage.startsWith ("POBEDNIK:"))
	{
	  ...
	  if (pobednik == 11)
		...
		{
		  const QString poruka
			  = "Oba odgovora su podjednako udaljena od rešenja.";
		  getUi ()->labTacanOdgovor->setText (poruka);
		  getUi ()->labTacanOdgovor->setAlignment (Qt::AlignCenter);
		  getUi ()->labTacanOdgovor->setStyleSheet ("background-color:green");
