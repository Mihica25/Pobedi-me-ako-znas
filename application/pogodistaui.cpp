...
PogodiStaUI::PogodiStaUI (QWidget *parent)
	: QWidget (parent), ui (new Ui::PogodiStaUI), pogodiSta (new PogodiSta ()),
...
PogodiStaUI::PogodiStaUI (QWidget *parent, QTcpSocket *tcpSocket,
						  QString player_1, QString player_2, bool red,
						  int pSPartija, int player1_points,
						  int player2_points)
...
QString
PogodiStaUI::getPlayer1 ()
{
  return player1;
}

QString
PogodiStaUI::getPlayer2 ()
{
  return player2;
}

void
PogodiStaUI::setPlayer1 (QString playerr1)
{
  player1 = playerr1;
}

void
PogodiStaUI::setPlayer2 (QString playerr2)
{
  player2 = playerr2;
}

void
PogodiStaUI::setPlayer1Points (int playerr1Points)
{
  player1Points = playerr1Points;
}

void
PogodiStaUI::setPlayer2Points (int playerr2Points)
{
  player2Points = playerr2Points;
}

int
PogodiStaUI::getRoundNumber ()
{
  return roundNumber;
}

int
PogodiStaUI::getPixelSize ()
{
  return pixelSize;
}

PogodiStaUI::~PogodiStaUI () { delete ui; }

void
PogodiStaUI::startGame ()
{
  pixelSize = 32;
  gameTime = 60;
  roundNumber = 6;
  ...
  if (pogodiStaPartija == 0)
	{
	  connect (ui->leInsert, &QLineEdit::returnPressed, this,
			   &PogodiStaUI::onGuessSubmit);
	}
	  ...
	  if (pixelSize == 0)
	  ...
	  else
		{
		  QPixmap updatedImage
			  = pixelateImage (pogodiSta->getGuessingImage (), pixelSize);
...
QPixmap
PogodiStaUI::pixelateImage (const QPixmap original, int pixelSize)
{
...
void
PogodiStaUI::sendMessage (QTcpSocket *socket, QString msg)
{
...
void
PogodiStaUI::processServerMessage (QString serverMessage)
{
  if (serverMessage.startsWith ("IMAGE_G:"))
	...
	{
	  disconnect (server, &QTcpSocket::readyRead, this,
				  &PogodiStaUI::onReadyRead);
	  disconnect (gameTimer, &QTimer::timeout, this,
				  &PogodiStaUI::updateTimer);
	  disconnect (this, &PogodiStaUI::gameEnds, this,
				  &PogodiStaUI::on_gameEnds);
	  sleep (3000);
	  emit mGameEnds ();
	}
...
void
PogodiStaUI::generateImage ()
{
  sendMessage (server, "IMAGE_GEN\n");
}

int
PogodiStaUI::getPlayer1Points ()
{
  return player1Points;
}

int
PogodiStaUI::getPlayer2Points ()
{
