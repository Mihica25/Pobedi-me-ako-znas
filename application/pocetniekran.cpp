...
PocetniEkran::PocetniEkran (QWidget *parent)
	: QMainWindow (parent), ui (new Ui::PocetniEkran)
{
  ui->setupUi (this);
  tcpSocket = nullptr;
...
int
PocetniEkran::getPlayer1Points ()
{
  return player1Points;
}

int
PocetniEkran::getPlayer2Points ()
{
  ...
  if (connectToServer ("SEND_BEST_RESULTS"))
	{
	  qDebug () << "Uspesno povezivanje sa serverom!";
	  connect (tcpSocket, &QTcpSocket::readyRead, this,
			   &PocetniEkran::onReadyReadBestResults);
	}
  else
	{
	  qDebug () << "Neuspesna konekcija sa serverom!";
...
bool
PocetniEkran::connectToServer (QString message)
{
...
void
PocetniEkran::processServerMessage (const QString &serverMessage)
{
...
PocetniEkran::initConntroler ()
{
  ...
  recko
	  = new ReckoUI (nullptr, tcpSocket, playerName, opponentName, turn, 0, 0);
  ...
  connect (recko, &ReckoUI::mGameEnds, this, &PocetniEkran::on_reckoEnds,
		   Qt::UniqueConnection);
}

void
PocetniEkran::on_reckoEnds ()
{
  ...
  mojbroj
	  = new Mojbroj (nullptr, tcpSocket, playerName, opponentName, turn,
					 recko->getPlayer1Points (), recko->getPlayer2Points ());
  ...
  connect (mojbroj, &Mojbroj::mGameEnds, this, &PocetniEkran::on_mojbrojEnds,
		   Qt::UniqueConnection);
...
PocetniEkran::on_mojbrojEnds ()
{
  ...
  kozna = new KoZnaui (nullptr, tcpSocket, playerName, opponentName, turn,
					   mojbroj->getPlayer1Points (),
					   mojbroj->getPlayer2Points ());
  kozna->setAutoFillBackground (true);
  stackedWidget->addWidget (kozna);
  stackedWidget->removeWidget (mojbroj);
  stackedWidget->setCurrentWidget (mojbroj);
  connect (kozna, &KoZnaui::mGameEnds, this, &PocetniEkran::on_koZnaEnds,
		   Qt::UniqueConnection);
}

void
PocetniEkran::on_koZnaEnds ()
{
  ...
  pogodiSta = new PogodiStaUI (nullptr, tcpSocket, playerName, opponentName,
							   turn, 0, kozna->getPlayer1Points (),
  ...
  connect (pogodiSta, &PogodiStaUI::mGameEnds, this,
		   &PocetniEkran::on_pogodiStaEnds);
}

void
PocetniEkran::on_podrundaEnded ()
{
  return;
}

void
PocetniEkran::on_memorijaEnds ()
{
...
PocetniEkran::on_pogodiStaEnds ()
{
  ...
  memorija = new Memorija (nullptr, tcpSocket, playerName, opponentName, turn,
						   pogodiSta->getPlayer1Points (),
  ...
  connect (memorija, &Memorija::mGameEnds, this,
		   &PocetniEkran::on_memorijaEnds);
}
