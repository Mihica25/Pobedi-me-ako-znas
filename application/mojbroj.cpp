	...
	: QWidget (parent), ui (new Ui::Mojbroj), timer (new QTimer (this))
{
  ...
  connect (ui->pushButton_num1, SIGNAL (released ()), this,
		   SLOT (buttonPressedNum ()));
  connect (ui->pushButton_num2, SIGNAL (released ()), this,
		   SLOT (buttonPressedNum ()));
  connect (ui->pushButton_num3, SIGNAL (released ()), this,
		   SLOT (buttonPressedNum ()));
  connect (ui->pushButton_num4, SIGNAL (released ()), this,
		   SLOT (buttonPressedNum ()));
  connect (ui->pushButton_num5, SIGNAL (released ()), this,
		   SLOT (buttonPressedNum ()));
  connect (ui->pushButton_num6, SIGNAL (released ()), this,
		   SLOT (buttonPressedNum ()));

  connect (ui->pushButton_add, SIGNAL (released ()), this,
		   SLOT (buttonPressedOp ()));
  connect (ui->pushButton_sub, SIGNAL (released ()), this,
		   SLOT (buttonPressedOp ()));
  connect (ui->pushButton_mul, SIGNAL (released ()), this,
		   SLOT (buttonPressedOp ()));
  connect (ui->pushButton_div, SIGNAL (released ()), this,
		   SLOT (buttonPressedOp ()));
  connect (ui->pushButton_leftBr, SIGNAL (released ()), this,
		   SLOT (buttonPressedOp ()));
  connect (ui->pushButton_rightBr, SIGNAL (released ()), this,
		   SLOT (buttonPressedOp ()));

  connect (ui->pushButton_submit, SIGNAL (released ()), this,
		   SLOT (buttonPressedSubmit ()));

  connect (ui->pushButton_nextRound, SIGNAL (released ()), this,
		   SLOT (buttonPressedNextRound ()));

  connect (ui->pushButton_del, SIGNAL (released ()), this, SLOT (del ()));
}

Mojbroj::Mojbroj (QWidget *parent, QTcpSocket *tcpSocket, QString player1_,
				  QString player2_, bool red, int player1Points_,
				  int player2Points_)
	: QWidget (parent), ui (new Ui::Mojbroj), timer (new QTimer (this))
{
  server = tcpSocket;
  ...
  connect (ui->pushButton_nextRound, SIGNAL (released ()), this,
		   SLOT (buttonPressedNextRound ()));
  connect (ui->pushButton_nextGame, SIGNAL (released ()), this,
		   SLOT (buttonPressedNextGame ()));
...
Mojbroj::buttonPressedSubmit ()
{
  ...
  sendMessage (server, "EXPRESSION:" + player1 + "%" + expression + "%"
						   + QString::number (result) + "\n");

  sendMessage (server,
			   "RESULT:" + player1 + "%" + QString::number (result) + "\n");
...
void
Mojbroj::buttonPressedNextRound ()
{
...
void
Mojbroj::setButtonStatus (bool enabled)
{
...
void
Mojbroj::processServerMessage (QString serverMessage)
{
...
void
Mojbroj::sendMessage (QTcpSocket *socket, QString msg)
{
